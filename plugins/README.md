...
| [multi-model-cli](./multi-model-cli/) | Integrate Claude Code with OpenAI Codex CLI and Google Gemini CLI through their OAuth logins | **Commands:** `/multi-model-cli:setup`, `/multi-model-cli:codex`, `/multi-model-cli:gemini`, `/multi-model-cli:status` - Set up, query, and manage external model CLIs<br>**Hook:** SessionStart - Detects available CLIs and informs about multi-model support<br>**Skill:** `multi-model-usage` - Guidance on using external model CLIs |
